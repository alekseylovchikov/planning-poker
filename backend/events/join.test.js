...
describe("join", () => {
  ...
  it("отклоняет roomId длиной > 7", async () => {
    const ws = makeWs();

    await join(ws, { payload: { name: "Alice", roomId: "12345678" } });
  ...
  it("переподключает участника с тем же именем", async () => {
    rooms.set("room1", {
      participants: [
        { id: "u1", name: "Alice", isOnline: false, hasVoted: false },
      ],
      votesRevealed: false,
      currentVotes: {},
      roomId: "room1",
    ...
    await join(newWs, { payload: { name: "Alice", roomId: "room1" } });

    expect(newWs.userId).toBe("u1");
    expect(oldWs.close).toHaveBeenCalled();

    const participant = rooms.get("room1").participants.find((p) => p.id === "u1");
    expect(participant.isOnline).toBe(true);
  });

  it("переподключение нечувствительно к регистру имени", async () => {
    rooms.set("room1", {
      participants: [
        { id: "u1", name: "Alice", isOnline: false, hasVoted: false },
      ],
    ...
    await join(ws, { payload: { name: "alice", roomId: "room1" } });

    expect(ws.userId).toBe("u1");
    expect(rooms.get("room1").participants).toHaveLength(1);
  });
  ...
  it("отправляет начальное состояние клиенту и делает broadcast", async () => {
    const ws = makeWs();
    vi.mocked(generateId).mockReturnValueOnce("room1").mockReturnValueOnce("user1");
